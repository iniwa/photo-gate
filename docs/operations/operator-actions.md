...
### A-1. 【最優先・本番が同期停止中】R2 アクセスキーの再発行
...
> **重要**: 現状はカバー画像も含め、本番の閲覧は正常。これは
> **fail-closed 設計**により「失敗した同期は R2 を一切書き換えない」
> ため、最後に成功した 0.1.6 同期(234 枚)の状態がそのまま残って
> いるから。**データ破損やリークは起きていない。** 急ぐ理由は
> 「新しい写真が反映されない・カバー以外の更新が止まっている」点のみ。
