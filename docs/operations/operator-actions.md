...
AGENTS.md の approval gates に従い、remote/live mutation、destructive 操作、
シークレット変更、Cloudflare Access の設定、デプロイ、または本番データへの
アクセスは、現在のユーザーによる明示的な承認が必要です。人間向けの手順を
記載していること自体は、その承認を代替しません。
...
以下の操作は AGENTS.md の approval gates により、現在のユーザーの
明示的な承認なしにエージェントが単独で実施することを禁止されています。
...
有効化するには:
1. 安全削除設計を別途レビューし、現在のユーザーの明示的な承認を取得する。
...
| 機能 | 状態 | 有効化条件 |
|---|---|---|
| R2 実削除 | 意図的に無効 | 別途の安全削除設計レビュー、現在のユーザーの明示的な承認、専用ハンドオフ (§C-2) |
...
- R2 削除は別途レビューと現在のユーザーの明示的な承認が完了するまで dry-run のみ。
