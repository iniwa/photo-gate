...
| 2026-06-30 | `0.4.2` | `fb57196` / `sync-v0.4.2` | 2026-06-30 | Reupload suppression release. `main` and `sync-v0.4.2` are pushed to Gitea at `fb57196`; operator reports Docker deploy and Portainer stack update to `0.4.2` are complete. Docker sync now emits manifest `schemaVersion: 2` with per-photo `sourceHash` and can skip unchanged thumb/preview pairs on subsequent syncs while still uploading cover and manifest. Live two-run skip smoke remains the next confirmation item. |
