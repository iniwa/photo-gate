...
- DONE: Commit `50bf499` implemented no-JS multi-select derived download links. `POST /download/:albumId/selection` validates selected manifest photo IDs and renders a private result page with individual existing thumb/preview download links. The POST route does not read thumb/preview object bodies, generate ZIPs, auto-download files, expose RAW/original links, or add JavaScript. Workers CI run `28642332470` succeeded and deployed Worker version `d469d24d-b52f-49cd-92f0-50bdd77c55cf`; unauthenticated smoke passed for `/`, `/albums`, `/img`, both individual `/download/.../thumb` and `/download/.../preview`, the new selection POST, `/api/probe`, and admin Access interception.
