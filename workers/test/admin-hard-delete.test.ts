...
describe('admin hard delete confirm routes', () => {
  ...
  it('renders album confirmation page with allowed summary only', async () => {
    ...
    expect(body).toContain('同期ターゲットを先に削除してから、アルバム行を D1 から削除します')
    expect(body).toContain('D1 の既存 foreign-key cascade により削除されます')
    expect(body).toContain('R2 アルバムオブジェクトは削除されません')
