...
| Admin hard-delete controls | `POST /admin/users/confirm-delete`, `POST /admin/users/delete`, `POST /admin/albums/confirm-delete`, `POST /admin/albums/delete` | Users: two-step HMAC confirmation then `DELETE FROM users WHERE id = ?` with D1 cascade for sessions/permissions. Albums: Phase 2 preview-only; no album D1 DELETE, no R2 delete, no sync-target mutation |
