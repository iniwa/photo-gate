...
| Admin hard-delete confirmation preview | `POST /admin/users/confirm-delete`, `POST /admin/users/delete`, `POST /admin/albums/confirm-delete`, `POST /admin/albums/delete` | D1 read-only target re-read + HMAC token validation only; Phase 2 preview returns not-yet-enabled page; no D1 DELETE, no session/permission mutation, no R2 delete, no sync-target mutation |
...
| `POST /admin/users/delete` | Verified + allowlisted + same-origin + form body (`token`, `phrase`) + valid unexpired user-delete token + phrase `DELETE USER` + target re-read | `200` not-yet-enabled result page; no D1 DELETE, no session/permission cascade |
| `POST /admin/albums/confirm-delete` | Verified + allowlisted + same-origin + form body (`albumId`) + `HARD_DELETE_HMAC_KEY` present | `200` confirmation-preview page with 15-minute HMAC token and exact phrase requirement; selects only `id`, `title`, `enabled`; no sync-target inspection and no actual delete |
| `POST /admin/albums/delete` | Verified + allowlisted + same-origin + form body (`token`, `phrase`) + valid unexpired album-delete token + phrase `DELETE ALBUM` + target re-read | `200` not-yet-enabled result page; no D1 DELETE, no R2 delete, no sync-target mutation |
| hard-delete preview POST routes | Missing/mismatched Origin, wrong Content-Type, invalid/repeated/extra fields, wrong phrase, malformed/tampered/expired/wrong-category token | `403` or `400` no-store; input is not reflected |
